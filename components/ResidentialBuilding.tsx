...
import {
  residentialAssetModels,
  type ResidentialAssetModel,
} from "@/lib/residential-assets";
...
const urls = residentialAssetModels.map((model) =>
  publicPath(`/assets/kenney/suburban/${model}.glb`),
);
const ready = new WeakMap<HTMLCanvasElement, Set<string>>();

export default function ResidentialBuilding({
  model,
  width,
  position,
  rotation = 0,
}: {
  model: ResidentialAssetModel;
  width: number;
  position: [number, number, number];
  rotation?: number;
}) {
  const assets = useGLTF(urls);
  const source = assets[residentialAssetModels.indexOf(model)].scene;
  ...
  useLayoutEffect(() => {
    ...
    set.add(model);
    gl.domElement.dataset.residentialModels = String(set.size);
    invalidate();
    return () => {
      set.delete(model);
      gl.domElement.dataset.residentialModels = String(set.size);
